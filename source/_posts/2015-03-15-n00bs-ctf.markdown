...
<p>Initially when we visit the levelfive.php page an xss alert were kept on popping up. So what we did is we ope `checked` the `Prevent this page from creating additional dialogs`. 
...
After we downloaded the file we started to listen, initially we heard so many Beeping sounds. It didnt strike me that its a Audio Morse code. After identifyint that its Morsecode we decoded the sound value with the help of `Morse View` windows based morse code decoder `[.. -. ..-. --- ... . -.-. ..-. .-.. .- --. .. ... -- --- .-. ... . -.-. --- -.. . - --- -. . ...]`. Finally we got a Flag called `Infosec_flagis_morsecodetones`. But we are not sure that this flag is the one for Level-15 because here we have DNS lookup but the flag is Morsecode. So Im Searching for the flag if anybody got the flag please comment me.....


* **CTF Flag for Level-15: [???Infosec_flagis_morsecodetones???]** 

Im keep on digging into all directories hope for the best. 
