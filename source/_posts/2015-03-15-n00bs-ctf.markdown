...
This level contains a DNS lookup command which is integrated with `Dig` command, where it retrives the `ns & mx` record if you give any domain name inside the dns lookup field. And also if you notice this page is vulnerable to command injection.
